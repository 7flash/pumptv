...
import type { WorldCharacter, WorldProp, WorldState } from "../shared/contracts.ts";
...
export const EMPTY_WORLD_STATE: WorldState = {
  ...
  locationDetails: "The opening shot has not established a durable location yet.",
  characters: [],
  props: [],
  openThreads: [],
  motifs: [],
...
export function normalizeWorldState(value: unknown, previous: WorldState = EMPTY_WORLD_STATE): WorldState {
  const parsed = WorldStateSchema.safeParse(value);
  if (!parsed.success) return previous;
  const next = parsed.data;

  return {
    revision: Math.max(previous.revision + 1, Math.floor(Number(next.revision || 0))),
    location: sanitizeLine(next.location, 120) || previous.location,
    locationDetails: sanitizeLine(next.locationDetails, 500) || previous.locationDetails,
    characters: next.characters.slice(0, 8).map((character: WorldCharacter) => ({
    ...
    props: next.props.slice(0, 10).map((prop: WorldProp) => ({
    ...
    lastEndingBeat: sanitizeLine(next.lastEndingBeat, 500) || previous.lastEndingBeat,
  };
}


export function normalizeReconciledWorldState(value: unknown, planned: WorldState): WorldState {
  const parsed = WorldStateSchema.safeParse(value);
  if (!parsed.success) return planned;
  const normalized = normalizeWorldState(parsed.data, { ...planned, revision: Math.max(0, planned.revision - 1) });
  return { ...normalized, revision: planned.revision };
}

export function parseWorldStateJson(value: string | null | undefined): WorldState | null {
  if (!value) return null;
  try {
    const parsed = WorldStateSchema.safeParse(JSON.parse(value));
    return parsed.success ? normalizeWorldState(parsed.data, { ...EMPTY_WORLD_STATE, revision: Math.max(0, parsed.data.revision - 1) }) : null;
  } catch {
    return null;
  }
}
...
export function worldStateForH3(state: WorldState) {
  ...
  return [
    ...
    state.visualRules.length ? `Visual invariants: ${state.visualRules.join(" | ")}` : null,
    state.motifs.length ? `Recurring motifs: ${state.motifs.join(" | ")}` : null,
