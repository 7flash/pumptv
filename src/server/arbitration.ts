...
export const GENERATION_LEAD_MS = Number(process.env.SLOP_GENERATION_LEAD_MS || 4_500);
...
async function roundWithProposals(row: any): Promise<PromptRound> {
  ...
  return {
    ...
    winnerProposalId: row.winnerProposalId == null ? null : Number(row.winnerProposalId),
...
export async function getOpenPromptRound(): Promise<PromptRound | null> {
  const row = await dbMeasure.measureSync("Load open prompt round", () =>
    db.promptRounds.select().where({ status: "open" }).orderBy("id", "DESC").first(),
  );
...
export async function ensurePromptRound(
  ...
  generationLeadMs = GENERATION_LEAD_MS,
) {
  const now = Date.now();
  const latestSafeClose = bufferUntilMs
    ? Math.max(now, bufferUntilMs - generationLeadMs)
    : now + PROMPT_WINDOW_MS;
  const closesAtMs = Math.max(now, Math.min(now + PROMPT_WINDOW_MS, latestSafeClose));
  ...
  if (existing?.targetEpisode === targetEpisode) {
    if (closesAtMs < existing.closesAtMs) {
      await dbMeasure.measureSync("Tighten prompt deadline", () =>
        db.promptRounds.select().where({ id: existing.id }).updateAll({ closesAtMs }),
      );
  ...
  if (existing) {
    ...
    await dbMeasure.measureSync("Expire stale proposals", () =>
      db.proposals.select().where({ roundId: existing.id, status: "open" }).updateAll({ status: "lost" }),
    );
  }
...
export async function castProposalVote(input: {
  ...
  if (!round || (round as any).status !== "open") throw new Error("Voting round is closed");
  if (Date.now() >= Number((round as any).closesAtMs)) throw new Error("Voting window has closed");

  const proposal = await dbMeasure.measureSync("Load vote proposal", () =>
    db.proposals.select().where({ id: input.proposalId, roundId: input.roundId }).first(),
  );
  if (!proposal || (proposal as any).status !== "open") throw new Error("Proposal is not votable");

  const existing = await dbMeasure.measureSync("Load existing proposal vote", () =>
    db.proposalVotes
      .select()
  ...
  if (existing) {
    ...
    try {
      ...
      const raced = await dbMeasure.measureSync("Reload raced proposal vote", () =>
        db.proposalVotes.select().where({ roundId: input.roundId, voterKey }).first(),
      );
      ...
      await dbMeasure.measureSync.assert("Move raced proposal vote", () =>
        db.proposalVotes
          ...
          .updateAll({ proposalId: input.proposalId, source: input.source, sourceId: input.sourceId ?? null }),
      );
    }
  }
...
export async function submitPromptProposal(input: {
  ...
  if (!round || (round as any).status !== "open") throw new Error("Prompt round is closed");
  if (Date.now() >= Number((round as any).closesAtMs)) throw new Error("Prompt window has closed");
  ...
  if (!proposal) {
    ...
    if (Number(count || 0) >= MAX_PROPOSALS) throw new Error("This prompt round is full");

    try {
      proposal = await dbMeasure.measureSync.assert("Create prompt proposal", () =>
        db.proposals.insert({
          roundId: input.roundId,
          text,
...
export async function commitPromptRound(targetEpisode: number) {
  const committed = dbMeasure.measureSync.assert("Commit prompt round winner", () => {
    ...
    try {
      ...
      db.exec(`UPDATE proposals SET status = 'lost' WHERE roundId = ? AND status = 'open'`, round.id);
      ...
      db.exec(`UPDATE proposals SET status = 'selected' WHERE id = ?`, winner.id);
